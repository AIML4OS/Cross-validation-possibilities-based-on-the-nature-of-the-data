...
outer_folds <- vfold_cv(data_cv1, v = 5)  # inappropriate dataset !!!! 
# WRONG CV STRATEGY for this data structure (ignores grouping)

outer_errors <- sapply(outer_folds$splits, function(split_outer) {

  train_outer <- analysis(split_outer)
  test_outer  <- assessment(split_outer)

  inner_folds <- vfold_cv(train_outer, v = 5)

  model_errors <- lapply(inner_folds$splits, function(split_inner) {

    train <- analysis(split_inner)
    test  <- assessment(split_inner)

    # --- FIX FACTORS (ważne!) ---
    facs <- names(train)[sapply(train, is.factor)]
    for (f in facs) {
      test[[f]] <- factor(test[[f]], levels = levels(train[[f]]))
    }

    # --- MODEL 1: mean ---
    pred1 <- mean(train$QF185001, na.rm = TRUE)
    err1 <- mean((test$QF185001 - pred1)^2, na.rm = TRUE)

    # --- MODEL 2: linear model ---
    m <- lm(QF185001 ~ ., data = train)
    pred2 <- predict(m, newdata = test)
    err2 <- mean((test$QF185001 - pred2)^2, na.rm = TRUE)

    c(err1 = err1, err2 = err2)
  })

  model_errors <- do.call(rbind, model_errors)
  avg_errs <- colMeans(model_errors)
  ...
  if (best_model == 1) {
    ...
    pred <- mean(train_outer$QF185001, na.rm = TRUE)
    test_err <- mean((test_outer$QF185001 - pred)^2, na.rm = TRUE)

  } else {

    # FIX FACTORS OUTER LEVEL TOO
    facs <- names(train_outer)[sapply(train_outer, is.factor)]
    for (f in facs) {
      test_outer[[f]] <- factor(test_outer[[f]], levels = levels(train_outer[[f]]))
    }

    m <- lm(QF185001 ~ ., data = train_outer)
    pred <- predict(m, newdata = test_outer)
